...
class Absents implements Feature {
  private onlyUniqueStatuses(
    value: HTMLElement,
    index: number,
    array: HTMLElement[]
  ) {
    ...
    for (let i = 0; i < index; i++) {
      const itemStateColor = array[i].classList[1];
      const valueStateColor = value.classList[1];
      if (valueStateColor === itemStateColor) {
        is = false;
        break;
      }
    }
  ...
  private getMonths() {
    const monthElements = document.querySelectorAll(
      ".absent-dropdown > li > a"
    );
    const monthList = [];
    for (let i = 1; i < monthElements.length; i++) {
      monthList.push(monthElements[i].innerHTML);
    }
    return monthList;
  }
  private makeTitle(title: string, statuses: Node[] = null) {
    const headerContainer = document.createElement("div");
    headerContainer.classList.add("flex-table");
    headerContainer.classList.add("header");
    headerContainer.classList.add("first");

    const headerTitle = document.createElement("div");
    headerTitle.classList.add("flex-row");
    headerTitle.classList.add("first");
    headerTitle.innerHTML = "".concat(title, " ");
  ...
  private makeMonth(title: string) {
    const details = document.createElement("details");
    const summary = document.createElement("summary");
    const tableContainer = document.createElement("div");
    tableContainer.classList.add("table-container");
    tableContainer.classList.add("absent-table");

    const monthContent = this.makeMonthContent(title);
    const content = monthContent[0] as HTMLDivElement;
    const statuses = monthContent[1] as Node[];
    const header = this.makeTitle(title, statuses);
  ...
  private makeMonthContent(month: string) {
    const content = document.createElement("div");
    content.classList.add("content");
    const montDayElements = document.querySelectorAll(
      '.table-container.absent-table[data-action-id="'.concat(month, '"]')
    );
    let monthStatuses: Node[] = [];

    for (let i = 0; i < montDayElements.length; i++) {
      const details = document.createElement("details");
      const summary = document.createElement("summary");
      ...
      let statuses: Node[] = [];
      const day = montDayElements[i];
      const absents = day.querySelectorAll(".row");

      for (let j = 0; j < absents.length; j++) {
        const absent = absents[j];
        dayContent.appendChild(absent);
        const status = absent.querySelector(".icon-circle")
        statuses.push(status.cloneNode());
        monthStatuses.push(status.cloneNode());
      }

      statuses = statuses.filter(this.onlyUniqueStatuses);
      const title = day
        .querySelector(".flex-table.header.first > div:only-child")
        .innerHTML.split(" - ")[1];
      const titleElement = this.makeTitle(title, statuses.slice());
    ...
    monthStatuses = monthStatuses.filter(this.onlyUniqueStatuses);
    return [content, monthStatuses];
  }
  show() {
    var months = this.getMonths();
    var mainContent = document.querySelector(".content");
    for (var i = 0; i < months.length; i++) {
      mainContent.appendChild(this.makeMonth(months[i]));
    }
  }
}
...
class Login implements Feature {
  ...
  private demoLogin(loginForm: HTMLElement, event: SubmitEvent) {
    ...
    alert(
      "Ovo je testno okruženje! Upozoravamo da neke funkcije možda neće raditi."
    );
  ...
  private handleRememberMe(loginForm: HTMLElement) {
    ...
    // @ts-ignore
    app.writeTmp(
      `${Login.getUsername(loginForm).value};${
        Login.getPassword(loginForm).value
      }`
    );
  }
...
function choseFeature() {
  ...
  if (
    ...
    location.pathname.indexOf("all") !== -1
  ) {
  ...
  if (location.pathname.indexOf("absent") !== -1) {
    const absents = new Absents();
    absents.show();
    return;
  }
}
...
function customLogo() {
  ...
  superLogoImg.setAttribute(
    "src",
    "https://pisoj.github.io/ed-super/img/logo-256.webp"
  );
